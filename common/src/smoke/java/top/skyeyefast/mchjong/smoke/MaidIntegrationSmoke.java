...
/** Real optional-mod entity and brain boundaries, shared by Fabric Orihime and NeoForge. */
final class MaidIntegrationSmoke {
    ...
    boolean tick(Minecraft client, BlockPos center, Path output) {
        ...
        work = client.getSingleplayerServer().submit(() -> {
            ...
            if (currentStep == 4) {
                var restored = EntityMaid.TYPE.create(level);
                restored.load(saved);
            ...
            if (currentStep == 3) {
                table.open(owner);
                saved = maid.saveWithoutId(new CompoundTag());
