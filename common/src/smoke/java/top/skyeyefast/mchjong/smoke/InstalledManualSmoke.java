        ...
        if (stage == 0) {
            ...
            crafted = client.getSingleplayerServer().submit(() -> {
                ...
                var recipe = (net.minecraft.world.item.crafting.CraftingRecipe) server.getRecipeManager()
                    .byKey(MahjongContent.id("mahjong_manual")).orElseThrow().value();
                ...
                var gifts = storage.computeIfAbsent(new net.minecraft.world.level.saveddata.SavedData.Factory<>(
                    top.skyeyefast.mchjong.compat.patchouli.ManualGift::new,
                    top.skyeyefast.mchjong.compat.patchouli.ManualGift::load, null), "mchjong_manual_gifts");
                storage.set("mchjong_manual_gifts", top.skyeyefast.mchjong.compat.patchouli.ManualGift.load(
                    gifts.save(new net.minecraft.nbt.CompoundTag(), server.registryAccess()), server.registryAccess()));
