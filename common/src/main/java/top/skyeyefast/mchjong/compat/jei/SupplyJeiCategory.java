...
final class SupplyJeiCategory implements IRecipeCategory<SupplyRecipeExample> {
    private final IRecipeType<SupplyRecipeExample> type;
    private final IDrawable icon;

    SupplyJeiCategory(IRecipeType<SupplyRecipeExample> type, IGuiHelper gui) {
        this.type = type;
        icon = gui.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Items.CRAFTING_TABLE));
    }

    @Override public IRecipeType<SupplyRecipeExample> getRecipeType() { return type; }
    ...
    @Override public Identifier getIdentifier(SupplyRecipeExample example) { return example.id(); }
    ...
    @Override public void setRecipe(IRecipeLayoutBuilder builder, SupplyRecipeExample example, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 100, 23).add(example.output());
