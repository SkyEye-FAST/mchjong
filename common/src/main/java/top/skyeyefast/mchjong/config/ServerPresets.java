...
public final class ServerPresets {
    ...
    private static Map<Identifier, PresetArchives.Stick> sharedSticks = Map.of();
    private static final Map<String, Identifier> appearances = new HashMap<>();
    private static final Identifier DEFAULT_STICK = Identifier.fromNamespaceAndPath("mchjong", "default");
    ...
    public static void chooseStick(ServerPlayer player, Identifier requested) {
        Identifier publicId = sharedSticks.containsKey(requested) || BuiltinPresets.STICKS.contains(requested)
        ...
        for (var target : player.level().getServer().getPlayerList().getPlayers())
