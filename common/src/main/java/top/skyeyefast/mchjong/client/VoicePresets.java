...
public final class VoicePresets {
    ...
    private record Definition(String name, Map<String, Identifier> recordings) {}
    private static Map<Identifier, Definition> local = Map.of(), server = Map.of();
    private static Map<Identifier, byte[]> localAudio = Map.of(), serverAudio = Map.of();
    private static volatile Map<Identifier, byte[]> audio = Map.of();
    private static List<Identifier> choices = List.of(DEFAULT);
    ...
    public static byte[] audio(Identifier path) { return audio.get(path); }
    public static Identifier audioPath(Identifier preset, String event) {
        var definition = server.getOrDefault(preset, local.get(preset));
        if (definition == null) return null;
        var sound = definition.recordings().get(event);
        return sound == null ? null : Identifier.fromNamespaceAndPath(sound.getNamespace(),
            "sounds/" + sound.getPath() + ".ogg");
    }

    public static void installLocal(Map<Identifier, PresetArchives.Voice> presets) throws IOException {
        var loaded = install(presets);
        local = loaded.definitions(); localAudio = loaded.audio(); update();
    }
    public static void installServer(Map<Identifier, PresetArchives.Voice> presets) throws IOException {
        var loaded = install(presets);
    ...
    private record Loaded(Map<Identifier, Definition> definitions, Map<Identifier, byte[]> audio) {}
    private static Loaded install(Map<Identifier, PresetArchives.Voice> presets) throws IOException {
        var definitions = new HashMap<Identifier, Definition>();
        var sounds = new HashMap<Identifier, byte[]>();
        for (var entry : presets.entrySet()) {
            if (DEFAULT.equals(entry.getKey())) throw new IOException("Reserved voice preset ID");
            var paths = new HashMap<String, Identifier>();
            for (var recording : entry.getValue().recordings().entrySet()) {
                ...
                var location = Identifier.fromNamespaceAndPath("mchjong", stem);
                var path = Identifier.fromNamespaceAndPath("mchjong", "sounds/" + stem + ".ogg");
    ...
    public static void trackChannel(SoundInstance sound, ChannelAccess.ChannelHandle handle, SoundBufferLibrary buffers) {
        ...
        if (handle != null && !sound.getSound().shouldStream()) {
            ...
            decoding.exceptionally(failure -> {
                org.slf4j.LoggerFactory.getLogger("mchjong").warn("Cannot decode voice {}", sound.getIdentifier(), failure);
                return null;
            });
        }
    }
    ...
    private static final class Recording extends AbstractSoundInstance {
        ...
        private Recording(Identifier location, float volume) {
