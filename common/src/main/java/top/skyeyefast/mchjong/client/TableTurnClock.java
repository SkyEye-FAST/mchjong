...
final class TableTurnClock extends AbstractWidget {
    ...
    @Override protected void extractWidgetRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, reserve, reserveX, 11,
            urgent ? MahjongUi.NEGATIVE : MahjongUi.ACCENT, true);
        graphics.pose().translate(reserveX - (reserve.isEmpty() ? 0 : 4) - font.width(move) * 2, 2);
        graphics.pose().scale(2, 2);
        graphics.text(font, move, 0, 0, urgent ? MahjongUi.NEGATIVE : usingReserve ? MahjongUi.ACCENT : MahjongUi.TEXT, true);
