...
/** A machine display: seven-segment scores, tile winds and seat lamps, without font labels. */
public final class TableIndicator {
    ...
    public static void render(TableView view, PoseStack pose, MultiBufferSource buffers, int light) {
        ...
        for (int seat = 0; seat < view.seats().size(); seat++) {
            ...
            number(pose, vertices, view.seats().get(seat).points(), .025f, .19f, .025f, DIGIT, light);
            wind(pose, vertices, Math.floorMod(seat - view.dealer(), view.rules().players()), -.119f, .202f, light);
            pose.popPose();
        }
        pose.pushPose();
        pose.mulPose(Axis.YP.rotationDegrees(Math.max(0, view.viewerSeat()) * 90));
        wind(pose, vertices, Math.min(3, view.round() / view.rules().players()), -.04f, -.061f, light);
        ...
        number(pose, vertices, view.remaining(), .03f, -.017f, .034f, DIGIT, light);
        panel(pose, vertices, -.104f, .065f, -.059f, .071f, LAMP, light);
        number(pose, vertices, view.honba(), -.026f, .05f, .026f, LAMP, light);
        panel(pose, vertices, .023f, .065f, .068f, .071f, DIGIT, light);
        number(pose, vertices, view.riichiSticks(), .102f, .05f, .026f, DIGIT, light);
        pose.popPose();
    }

    private static void wind(PoseStack pose, VertexConsumer out, int wind, float x, float z, int light) {
        pose.pushPose();
        pose.translate(x, SURFACE + .013, z);
        pose.mulPose(Axis.XP.rotationDegrees(-90));
        pose.scale(.29f, .29f, .29f);
        TileMesh.drawFace(pose, out, (27 + wind) * 4, false, light);
        pose.popPose();
    }

    private static void number(PoseStack pose, VertexConsumer out, int value, float center, float z,
                               float height, int color, int light) {
        ...
        for (char digit : digits.toCharArray()) {
            ...
            float t = height * .10f, middle = height / 2;
            float[][] bars = {
