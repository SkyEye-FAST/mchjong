# Documentation

[Project overview](../README.md) · [Changelog](../CHANGELOG.md)

MChjong is a Minecraft mahjong mod. The currently implemented rule family is
riichi, with three- and four-player modes.

## Playing

| Guide | Contents |
| --- | --- |
| [Playing at a table](PLAYING.md) | Setup, controls, views, settlements and invitations |
| [Survival equipment and recipes](SURVIVAL.md) | Furniture, boxes, tiles, dice and point sticks |
| [Rules and presets](RULES.md) | Current rules, preset defaults and custom settings |
| [Rooms and permissions](ROOMS.md) | Hosting, visibility, configuration and commands |
| [Replays](REPLAYS.md) | Playback, privacy, storage and Tenhou export |

## Resource customization

| Guide | Contents |
| --- | --- |
| [Assets](ASSETS.md) | Tile faces, backs, resource packs and deterministic generation |
| [Audio](AUDIO.md) | Table effects, recorded voices and resource-pack examples |

## Development and verification

| Guide | Contents |
| --- | --- |
| [Building and contributing](DEVELOPMENT.md) | Builds, development runs, checks and workflows |
| [Architecture](ARCHITECTURE.md) | Modules, ownership and shared contracts |
| [Interface style](UI_STYLE.md) | Widgets, layout, accessibility and visual acceptance |
| [Compatibility and verification](COMPATIBILITY.md) | Dependency profiles and integration checks |
| [Build download mirrors](BUILD_MIRRORS.md) | Repository and client asset downloads |
| [Ponder integration](PONDER.md) | Tutorials and installed-dependency smokes |
| [Training bots](BOTS.md) | Decision model, difficulty and comparison checks |

Contribution and release requirements are in [AGENTS.md](../AGENTS.md).
