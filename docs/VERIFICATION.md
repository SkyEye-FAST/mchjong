...
The batch synchronized from `main` at `a5d6852` and committed as `20e1597`
passed `buildAll` and the installed-maid fixture on Fabric and Forge. Full base
client checks also passed on 2026-09-24, producing fresh PASS markers at 15:23
and 15:28 local time respectively. These checks cover the updated preset picker,
paused last-departure decision, rejoining, physical handling, private inventory
and four-language UI. Maid checks cover recruitment, saved-entity recovery,
assigned mounts, legal play and task-change cleanup.

The earlier `d4399bb` Create batch records native machine transactions, installed
JEI/EMI and four-language Ponder playback on both loaders. Earlier viewer batches
also validated REI's finite recipe pages and 1,584 examples with 23,664 ingredient
alternatives. Those baselines are not a claim that every optional profile was
rerun for this documentation change. EMI's own synthetic `emi:brewing/` entries
may be absent from the vanilla recipe manager; the MChjong recipe checks inspect
MChjong entries separately.

Quilt Loader 0.30.1 previously loaded the packaged 1.20.1 Fabric JAR on Java 17
to the title screen. Gameplay acceptance above is scoped to Fabric and Forge.
...
This profile passed `buildAll --warning-mode fail` and both smoke compilation
tasks on JDK 21, retaining Java 17 production targets. Fabric and Forge passed
focused `smokeInterface`, `smokeSettlement` and `smokeVisibility` runs, covering
synchronized preset archives, the authorized box-menu lifecycle, sequential score
announcements and hand visibility. Fresh PASS markers and preset, settlement and
immersive screenshots were inspected.

Both loaders also passed `-PwithCreate=true -PwithPonder=true -PsmokeCreate=true
-PrecipeBrowser=rei`, covering native workshop transactions, recipe lookups and
four-language tutorial playback. Normal and small-window workshop screenshots
were inspected. Additional `-PwithCreate=true -PsmokeBrowser=true
-PrecipeBrowser=rei` captures confirm the animated workshop recipe category on
both loaders, under `build/smoke/browser-rei-evidence`. Evidence is under each loader's
`build/smoke/{interface,settlement,visibility,rei}-evidence` directory; logs use
`build/compat-*.log`. Quilt and other optional profiles retain their separately
dated runtime records.
